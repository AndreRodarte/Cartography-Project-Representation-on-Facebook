...
df_ads_combined$NM_MUNICIPIO_NASCIMENTO = stri_trans_general(str = df_ads_combined$NM_MUNICIPIO_NASCIMENTO, id = "Latin-ASCII")   #removing accents
df_ads_combined$NM_MUNICIPIO_NASCIMENTO <- tolower(df_ads_combined$NM_MUNICIPIO_NASCIMENTO) #lower case
```
...
IBGE_ads_log$Candidate_Born <- log(IBGE_ads_log$Candidate_Born + 1)
...
```{r}
Model_ads_Geo <- lm(Ads_Count ~ Population + GINI + Medium_Salary + Demographic_Density + factor(Candidate_Born),
                       data = IBGE_ads_log) 
```
