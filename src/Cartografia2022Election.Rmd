...
library(sysfonts)
library(showtext)
# Add EB Garamond font from Google Fonts
...
We combined the TSE data set with information about the candidates' communications on social media, specifically their account names, number of views and number of interactions.

```{r, message=FALSE, warning=FALSE}
# First, we created a dataset combining all interactions these candidates received during the election
...
**Descriptive analysis of the entire sample** Our sample created a total of 73,159 Facebook posts.
...
df_total_arranged %>%
  ...
  ylab("") +
  theme(text = element_text(family = "ebgaramond", face = "bold", size = 25),  
...
# **RQ1: What congressional candidates were absent from social media?**

```{r}
df_nas <- df[is.na(df$page_name), ] # 659 candidates did not have a public account on Facebook.
```
...
# **RQ2: Is the distribution of social media metrics of engagement among congressional candidates consistent with the normalization hypothesis?**
We previously noted that 659 candidates did not have a public account on Facebook. Now, we will focus on the 349 candidates who had an account and that created at least one post during the campaign (i.e. had an active account)

```{r}
# total_accounts <- 1018
df_clean <- df %>%
  filter(!is.na(N_Posts)) 
unique_page_names <- length(unique(df_clean$page_name))
# Calculate the percentage of candidates with a social media account
percentage_with_social_media <- (unique_page_names / total_accounts) * 100
```
349 candidates had an account and created at least one post during the campaign, representing 34% of our sample.
...
# **RQ3: Do congressional candidates target municipalities in a way that is consistent with their claims to scatter in the corners of the nation?** 

Here we are interested in which were the constituencies (geographically defined) parliamentarians reached out to the most and, conversely, what constituencies were left aside in this election.
...
#Please note that read_municipality() relies on an Internet connection. It does not throw an error without a connection.
...
muni3 <- muni3 %>%
  ...
  mutate(n = ifelse(name_muni == "contagem", 810, n)) %>%
...
#Please check "AmbiguousNames.RMD" for our analysis of posts referencing municipalities with ambiguous names. 
...
```{r}
ggplot(data = IBGE) +
  geom_jitter(aes(x = n, y = GINI, color = Mesoregion)) +
  labs(title = "Relationship between mentions and Human Development Index", 
       x = "Mentions", 
       y = "Human Development Index") +
  theme(
    text = element_text(family = "ebgaramond", face = "bold", size = 25),
...
```{r}
ggplot(data = IBGE) +
  geom_jitter(aes(x = n, y = Population, color = Mesoregion)) +
  labs(title = "Relationship between mentions and population", 
       x = "Mentions", 
       y = "Population") +
  theme(
    text = element_text(family = "ebgaramond", face = "bold", size = 25),
...
```{r}
ggplot(data = IBGE) +
  geom_jitter(aes(x = n, y = PIB_per_capita, color = Mesoregion)) +
  labs(title = "Relationship between mentions and PIB", 
       x = "Mentions", 
       y = "PIB per capita") +
  theme(
    text = element_text(family = "ebgaramond", face = "bold", size = 25),
...
BV <- ggplot() +
  ...
  labs(title = "Relationship between mentions and population (estimated for 2021)", 
       x = "Mentions", 
       y = "Population") +
  guides(fill = guide_colourbar(title = "Mentions")) +
  theme(
    text = element_text(family = "ebgaramond", face = "bold", size = 15),
    plot.title = element_text(hjust = 0.5) # Optional: center the title
  )

# Convert the ggplot object to an interactive plotly object
InteractivePopulation <- ggplotly(BV, tooltip = "text") %>%
  layout(
    title = list(font = list(family = "EB Garamond", size = 15, face = "bold")),
    xaxis = list(titlefont = list(family = "EB Garamond", size = 10)),
    yaxis = list(titlefont = list(family = "EB Garamond", size = 10))
  )
...
```{r}
df_clean$NM_MUNICIPIO_NASCIMENTO = stri_trans_general(str = df_clean$NM_MUNICIPIO_NASCIMENTO, id = "Latin-ASCII") 
df_clean$NM_MUNICIPIO_NASCIMENTO <- tolower(df_clean$NM_MUNICIPIO_NASCIMENTO) 

muni_under_rep <- IBGE %>%
  filter(n < 10) %>% 
  select(name_muni, n, Name)

df_origin <- left_join(muni_under_rep, select(df_clean, page_name, NM_MUNICIPIO_NASCIMENTO), by = c("name_muni" = "NM_MUNICIPIO_NASCIMENTO"))

df_origin2 <- df_clean %>%
  group_by(NM_MUNICIPIO_NASCIMENTO) %>%
  summarize(page_name = paste(page_name, collapse = ", ")) %>%
  ungroup()

df_origin3 <- left_join(muni_under_rep, df_origin2, by = c("name_muni" = "NM_MUNICIPIO_NASCIMENTO")) %>%
  filter(page_name!="NULL")
...
IBGE$Candidate_Born <- ifelse(IBGE$name_muni %in% df_clean$NM_MUNICIPIO_NASCIMENTO, 1, 0)
IBGE_log$Candidate_Born <- ifelse(IBGE_log$name_muni %in% df_clean$NM_MUNICIPIO_NASCIMENTO, 1, 0)
  
```

Finally, we can run our statistical test to determine which municipalities were targeted the most.
...
# library(sandwich)
# 
# # Perform hypothesis test (e.g., Wald test)
# waldtest(model_born, vcov = vcovHC(model_born, type = "HC1"))
```

The p-value provides strong evidence to suggest that including the variable Candidate_Born significantly improves the model's fit compared to the null model without any predictors, so we can reject the null hypothesis.

This implies that having a candidate born in a particular municipality is associated with a statistically significant difference in the count of mentions (n) in social media posts.
...
The odds ration provides estimated incidence rate ratios (IRRs) for the coefficients in the Poisson regression model. For the Candidate_Born, the estimated IRR is approximately 6.69. This means that, holding all other variables constant, having a candidate originally from a specific municipality increases in about 6.69 times the count of mentions (n) compared to not having a candidate from the same city.

### Geographical visual analysis

Based on the number of times municipalities *mineiras* were mentioned, we plotted an interactive choropleth map. The map allows the reader to hoover over the municipalities and see their names and the number of times they were mentioned.
...
MapBlack <- ggplot() +
  geom_sf(
    ...
    aes(
      fill = n, 
      text = paste0(name_muni, "<br>Population: ", Population, "<br>Mentions: ", n),  # Include mentions (n) in hover text
      geometry = geom
    ), 
    color = NA, size = .15
  ) +
  ...
  theme_minimal() +
  theme(
    text = element_text(family = "ebgaramond", face = "bold", size = 10),
    axis.line = element_blank(), 
    axis.text = element_blank(),
    axis.ticks = element_blank(), 
    axis.title = element_blank(),
    panel.grid = element_blank(),
    legend.title = element_text(family = "ebgaramond", face = "bold", size = 10),  # Color bar title
    legend.text = element_text(family = "ebgaramond", size = 8)  # Color bar labels
  ) +
...
# Create the interactive map with hover text in EB Garamond
InteractiveMapBlack <- ggplotly(MapBlack, tooltip = "text") %>%
  layout(
    title = list(
      text = paste0("<B>2022 Election: municipalities mentioned</B>"),
      font = list(family = "EB Garamond", size = 16)
    ),
    hoverlabel = list(
      font = list(family = "EB Garamond", size = 12)
    )
  )
